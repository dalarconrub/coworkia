...
rem Ejecuta la apertura bajo 1Password. Si `op` no tiene sesion, veras el error.
op run --env-file="%ENV_FILE%" -- cmd /c "apps\abrir_sesion.bat"
set "EXIT_CODE=%errorlevel%"

if not "%EXIT_CODE%"=="0" (
    echo.
    echo [WARN] La apertura termino con codigo %EXIT_CODE%.
